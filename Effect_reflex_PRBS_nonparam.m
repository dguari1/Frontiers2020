...
for m = 1 : 100 %100 simulation trials 
    for k =1 : length(gainn)
        ...
        IRF{k}(m,:) = hIntrinsic.dataSet;
        est= double(nlsim(hIntrinsic,nldat(pos,'domainIncr',1/100)));
    ...
    %     Iner_var(k,1) = sqrt(COV(3,3));
        ...
        %parameter estimation - parametric model
        %[Params_int,Ps_int,est] = my_rivbjmiso(decimate(Torque,10),[decimate(input_position(1:N),10) decimate(velocity(1:N),10) decimate(acceleration(1:N),10)],0,ones(1,3));
...
% 
        ...
        mean_reflex(k,m)=mean(Reflex_torque);
    end
...
hold on
errorbar(mean(RtoI'), Mean_Elast', 2*Std_Elast')
title('Elasticity')
xlabel('ratio_{RtoI}')
ylabel('K (Nm/rad)')
...
hold on
errorbar(mean(RtoI'), Mean_Visc, 2*Std_Visc)
title('Viscosity')
xlabel('ratio_{RtoI}')
ylabel('B (Nm/rad/s)')
...
hold on
errorbar(mean(RtoI'), Mean_Iner', 2*Std_Iner')
title('Inertia')
xlabel('ratio_{RtoI}')
ylabel('I (Nm/rad/s^2)')
